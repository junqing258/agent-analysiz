...
> 一个 Claude Code 技能插件，输入代码或名称即可自动生成专业级决策看板。支持 A股个股、港股、美股、A股指数、美股指数、公募基金。
...
| **智能数据源**    | 分级降级策略，Tushare（token-gated）/EastMoney/Yahoo 直连 |
| **节假日判断**    | 工作日判断（周一至周五），支持交易日查询 |
...
将本项目克隆到 Claude Code 的 skills 目录（注意目录名必须是 `stock-analysis`，与 SKILL.md 中的 `name` 字段一致）：
...
```bash
mv Stock-Analysis-Skill-main ~/.claude/skills/stock-analysis
```

数据脚本为**零依赖 Node**（`references/stock_data_fetcher.mjs`），仅需 Node ≥18（内置 `fetch`），无需安装任何 Python 包 / uv / pip / venv。

> 可选增强数据源（Tushare/Tavily/SerpAPI）在 `.env` 配置对应 API Key 后启用；未配置则**静默降级**到免费档 — 查看 JSON 输出的 `data_sources` 字段可诊断。
...
[STEP 1] 解析输入 → 识别市场（A股个股/港股/美股/A股指数/美股指数/公募基金），标准化代码
      │
      ▼
[STEP 2] Node 脚本获取数据 → 实时行情 + 120日K线 + 技术指标计算
      │
      ▼
...
公募基金:  EastMoney 基金接口（NAV 走势 + 基本面 + 持仓，数据源唯一）
```
...
工作日判断（周一至周五），不依赖 chinese_calendar，不识别中国法定节假日与调休。

**节假日功能说明**：

脚本在股票分析输出中包含 `trading_day_status` 字段（工作日判断；中国法定节假日落在工作日时会被判为交易日）：
...
| 市场 | 优先级 | 数据源 | 方式 | 费用 |
| ---- | ------ | ------ | ---- | ---- |
| A股个股 | P0 | Tushare Pro | HTTP POST（token-gated） | 免费（需注册） |
| A股个股 | P1 | 东方财富 | HTTP JSON 直连 | 免费 |
| A股个股 | P2 | Yahoo Finance | HTTP JSON 直连 | 免费 |
| 港股 | P1 | 东方财富 | HTTP JSON 直连 | 免费 |
| 港股 | P2 | Yahoo Finance | HTTP JSON 直连 | 免费 |
| 美股 | P0 | Yahoo Finance | HTTP JSON 直连 | 免费 |
| A股指数 | P0 | 东方财富 | HTTP JSON 直连 | 免费 |
| A股指数 | P1 | Yahoo Finance（仅少数有映射） | HTTP JSON 直连 | 免费 |
| 美股指数 | P0 | Yahoo Finance | HTTP JSON 直连 | 免费 |
| 公募基金 | P0 | 东方财富基金接口（净值 + 基本面 + 持仓） | HTTP HTML/JSON 直连 | 免费 |
...
│   ├── references/
│   │   ├── stock_data_fetcher.mjs         # 零依赖 Node：数据获取 + 技术指标计算
│   │   ├── strategy.json                  # 评分权重 / 信号阈值 / 硬规则配置
