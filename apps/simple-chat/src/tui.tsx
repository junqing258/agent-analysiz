...
import { Box, render, Text, useApp, useCursor, useInput } from "ink";
...
function ChatApp({ options }: { options: TerminalChatOptions }): React.JSX.Element {
  ...
  setCursorPosition(
    busy
      ? undefined
      : {
          x: 2 + displayWidth(input.slice(0, cursor)),
          y: terminalHeight - 4,
        },
  );
  ...
  const submit = useCallback(async () => {
    ...
    if (text === "/help") {
      appendSystemMessage(
        "Commands: /help, /clear, /exit. Enter sends; Ctrl-C or Ctrl-D exits.",
      );
      return;
    }
    if (text === "/clear") {
  ...
  useInput((keyInput, key) => {
    const isBackspace = key.backspace || keyInput === "\u007F" || keyInput === "\b";
    const isDelete = key.delete || keyInput === "\u001B[3~";
    ...
    <Box flexDirection="column" height={terminalHeight} width="100%">
      ...
      >
        <Box flexGrow={1} justifyContent="center">
          <Composer value={input} busy={busy} />
        </Box>
      </Box>
      <Box paddingX={1} backgroundColor="blue">
        <Text color="white" dimColor wrap="truncate-end">
          {busy
            ? "Generating… Ctrl-C exits"
            : "Enter send · Ctrl-U clear input · /help commands · Ctrl-C exit"}
        </Text>
      </Box>
    </Box>
  );
...
function Composer({ value, busy }: { value: string; busy: boolean }): React.JSX.Element {
  return (
    <Text color={busy ? "gray" : "white"} wrap="truncate-end">
      {busy ? "Assistant is responding…" : value}
    </Text>
  );
}
