...
export async function recoverSession(store: EventStore): Promise<RecoveredSession> {
  ...
  for (let index = 0; index < all.length; index += 1)
    if (all[index]?.event.type === "context.compacted") checkpoint = index;
  const snapshot =
    checkpoint >= 0 && all[checkpoint]?.event.type === "context.compacted"
      ? (all[checkpoint]?.event as Extract<DurableEvent, { type: "context.compacted" }>).snapshot
      : undefined;
  ...
  return {
    ...
    incompleteCallIds: [...requested].filter((id) => !completed.has(id)),
  };
}
  ...
  for (const callId of recovered.incompleteCallIds) {
    const result: ToolResult = {
      ...
      error: {
        ...
        message: "Tool was interrupted before completion; it was not retried.",
        retryable: true,
      },
      content: [{ type: "text", text: "TOOL_INTERRUPTED: tool was not automatically retried" }],
    };
