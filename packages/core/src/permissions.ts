...
export class DefaultPermissionPolicy implements PermissionPolicy {
  async evaluate({ tool, rules }: { tool: Tool; call: unknown; workspace?: string; rules: PermissionRule[] }): Promise<PermissionPolicyResult> {
    if (tool.risk === "read") return { type: "allow" };
    const now = Date.now();
    if (rules.some((rule) => rule.tool === tool.name && rule.risk === tool.risk && Date.parse(rule.expiresAt) > now)) return { type: "allow" };
    return { type: "ask" };
  }
}
...
export class InMemoryPermissionStore implements PermissionStore {
  ...
  async listRules(filter?: { workspace?: string; tool?: string }) {
    const now = Date.now();
    return [...this.rules.values()].filter((rule) => Date.parse(rule.expiresAt) > now && (!filter?.tool || rule.tool === filter.tool) && (!filter?.workspace || rule.scope.workspace === filter.workspace));
  }
  async revokeRule(id: string) { this.rules.delete(id); }
}
...
export function validatePermissionRule(rule: PermissionRule): void {
  if (!rule.id || !rule.tool || !Number.isFinite(Date.parse(rule.expiresAt)) || Date.parse(rule.expiresAt) <= Date.now()) throw new Error("Permission rule must have an identifier, tool, and future expiry");
  ...
  if (scope.pathPrefix === "/" || scope.pathPrefix === "*" || scope.executable === "*") throw new Error("Permission rule scope is too broad");
  if (scope.argsPrefix?.some((arg) => arg === "*")) throw new Error("Permission rule argv scope may not contain wildcards");
}
