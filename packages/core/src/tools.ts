...
export class ToolRegistry {
  ...
  constructor(tools: Iterable<Tool> = []) { for (const tool of tools) this.register(tool); }
  ...
  definitions(): ModelToolDefinition[] { return [...this.byName.values()].map(({ name, description, inputSchema }) => ({ name, description, inputSchema })); }
}
