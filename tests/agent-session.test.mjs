...
class MemoryEventStore {
  ...
  async *readAfter(sequence) {
    yield* this.events.filter((entry) => entry.sequence > sequence);
  }
}
...
test("runs a streamed two-tool loop and never stores deltas", async () => {
  ...
  const gateway = new MockGateway([
    [
      { type: "text-delta", text: "I will check. " },
      { type: "tool-call-delta", id: "a", inputTextDelta: "{}" },
      { type: "tool-call", id: "a", name: "first", input: {} },
      { type: "finish", reason: "tool-use" },
    ],
    [
      { type: "tool-call", id: "b", name: "second", input: { value: "x" } },
      { type: "finish", reason: "tool-use" },
    ],
    [
  ...
  assert(
    !store.events.some(({ event }) => event.type === "model.text.delta" || event.type === "model.tool-call.delta"),
  );
...
test("hangs on policy ask and resumes after one resolved permission", async () => {
  ...
  const gateway = new MockGateway([
    [
      { type: "tool-call", id: "edit-1", name: "edit", input: {} },
      { type: "finish", reason: "tool-use" },
    ],
    [
  ...
  const session = new AgentSession({
    model: gateway,
    eventStore: store,
    permissions: { store: new InMemoryPermissionStore(), permissionTimeoutMs: 5_000 },
    tools: [
      defineTool({
        name: "edit",
        description: "edit",
...
test("turn cancellation records interruption and does not execute a later tool", async () => {
  ...
  const gateway = new MockGateway([
    [
      { type: "tool-call", id: "a", name: "slow", input: {} },
      { type: "finish", reason: "tool-use" },
    ],
  ]);
  let executed = false;
  ...
  for await (const _ of session.run("cancel", { signal: controller.signal })) {
    /* consume */
  }
...
test("compacts over-budget history into a durable checkpoint", async () => {
  ...
  const session = new AgentSession({
    model: gateway,
    eventStore: store,
    context: { system: "system", contextWindowTokens: 30, maxOutputTokens: 5, retainRecentMessages: 1 },
  });
  for (const text of [
    "a long first message that exceeds a tiny budget",
    "another large user message",
    "one more large user message",
  ])
    for await (const _ of session.run(text)) {
      /* consume */
    }
  const checkpoint = store.events.find(({ event }) => event.type === "context.compacted");
  assert(checkpoint);
  assert.equal(
    checkpoint.event.snapshot.messages.some((message) =>
      message.content.some((content) => content.type === "text" && content.text.startsWith("Conversation summary")),
    ),
    true,
  );
});
...
test("truncation preserves a tool failure instead of changing it to success", async () => {
  ...
  const gateway = new MockGateway([
    [
      { type: "tool-call", id: "bad", name: "bad", input: {} },
      { type: "finish", reason: "tool-use" },
    ],
    [{ type: "finish", reason: "stop" }],
  ]);
    ...
    tools: [
      defineTool({
        ...
        async execute() {
          return {
            ok: false,
            error: { code: "LONG_FAILURE", message: "x".repeat(200), retryable: false },
